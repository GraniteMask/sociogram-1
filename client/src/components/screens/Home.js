        ...
        <div className="home">
            {
                data.map(item=>{
                    ...
                    <h5 style={{padding:"5px"}} className="font"><Link to={item.postedBy._id!==state._id?"/profile/"+item.postedBy._id : "/profile"}>{item.postedBy.name}</Link> {item.postedBy._id==state._id && <i className="material-icons" style={{float: "right", cursor:"pointer"}} 
                    ...
                    {item.likes.includes(state._id)
                        ...
                        {/* Comments */} {/*here, record=comment, item=post and state=logged in user*/}
                        {
                            item.comments.map(record=>{
                                return(
                                <h6 key={record._id}><span className="font" style={{fontWeight:"1000"}}>{record.postedBy.name}</span>: {record.text} 
                                
                                {record.postedBy._id==item.postedBy._id ? 
                                
                                item.postedBy._id==state._id && <i className="material-icons fonts" style={{float: "right", cursor:"pointer"}} 
